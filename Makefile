...
.PHONY: test enhance build lint-imports draw draw-run-detect draw-run-track draw-run-mp4
...
draw:
>docker build -f Dockerfile.draw -t decoder-draw:latest .
...
>  --output-dir /app/out/frames_viz --label --score-min 0.1
